            ...
            foreach (string str in Words)
            {
                ...
                else
                {
                    ...
                    else if (str[0] == '/') // Check for Character Constant
                    {
                        ...
                        Token[count, 0] = "CHARACTER_CONST";
                        Token[count, 1] = TempStr;
                        Token[count, 2] = Convert.ToString(Current_Line_Number);
                        count++;
                        continue;
                    }
                    else if  ((str[0] >= 65 && str[0] <= 90)  || str == "." || str == "{" || str == "}" || str == "(" || str == ")" || str == ":" || str == "," || str == "*" || str == "++" || str == "+" || str == "--" || str == "-")
                    {
                        //Check For Keywords
                        for (int j = 0; j <39; j++)
                        {
        ...
        string[] GetCurrentWords(string Line)
        {
            ...
            while (true)
            {
             ...
             TempACount = Counter;
                ...
                if (LineArr[k] == '.' || LineArr[k] == '{' || LineArr[k] == '}' || LineArr[k] == '(' || LineArr[k] == ')' || LineArr[k] == ':' || LineArr[k] == ',' || LineArr[k] == '*' || LineArr[k] == '/' || LineArr[k] == '+' || LineArr[k] == '-')
                {
                    bool Plus = false; bool Minus = false; bool Divide = false; bool Multiply = false;
                    // If +  then check for ++
                    if (LineArr[k] == '+' )
                    {
                        Plus = true;
        ...
        int IdentifiersKeywordsCheck(int k)
        {
            ...
            while (true)
            {
                ...
                if (k >= LineArr.Length)    // if line ends that is length is finished break loop
                    break;
                if (LineArr[k] == ' ' || LineArr[k] == '.' || LineArr[k] == '\"' || LineArr[k] == '\'' || LineArr[k] == '{' || LineArr[k] == '}' || LineArr[k] == '(' || LineArr[k] == ')' || LineArr[k] == ':' || LineArr[k] == ',' || LineArr[k] == '*' || LineArr[k] == '/' || LineArr[k] == '+' || LineArr[k] == '-')
